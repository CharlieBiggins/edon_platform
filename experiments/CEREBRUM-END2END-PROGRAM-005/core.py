...
PREDECESSOR = EXPERIMENTS / "CEREBRUM-END2END-PROGRAM-004"
...
def cfg():
    ...
    if c["arms"] != ["ordinary", "repair"] or c["binding_authority"] or c["transfer_authorized"] or c["confirmation_authorized"]:
    ...
    if c["train"]["ordinary_families"] * 8 != c["training_records"] or c["train"]["boundary_families"] * 12 != c["targeted_records"]:
        raise ValueError("generator/training count mismatch")
    for split in ("development",):
        s = c[split]
        if s["ordinary_families"] * 8 + s["boundary_families"] * 12 != s["records"]:
...
def inherited_sources():
    ...
    older = parse(pinned_bytes(PREDECESSOR / "registration.json", cfg()["predecessor_registration_sha256"]))
    sources = dict(reg["sources"])
    sources[PARENT.name + "/registration.json"] = cfg()["parent_registration_sha256"]
    sources[PREDECESSOR.name + "/registration.json"] = cfg()["predecessor_registration_sha256"]
    for name, expected in older["sources"].items():
        pinned_bytes(PREDECESSOR / name, expected)
        sources[PREDECESSOR.name + "/" + name] = expected
    for name in ("prepared/exclusions.json", "prepared/qualification.json"):
        expected = older["data"][name]
        pinned_bytes(PREDECESSOR / name, expected)
        sources[PREDECESSOR.name + "/" + name] = expected
    return sources
...
def dependencies():
    ...
    spec = importlib.util.spec_from_file_location("curriculum005_generator", common.SOURCE / "actionnet021.py")
...
def verify():
    ...
    if reg["protocol_id"] != cfg()["protocol_id"] or reg["confirmation_materialized"]:
        raise ValueError("registration identity or stage differs")
    if (ROOT / "prepared/confirmation.jsonl").exists() or (ROOT / "results/confirmation-access.json").exists():
        raise ValueError("Program-005 has no confirmation authorization")
    actual_data = {p.relative_to(ROOT).as_posix() for p in (ROOT / "prepared").rglob("*") if p.is_file()}
    if set(reg["data"]) != actual_data:
        raise ValueError("prepared data inventory changed")
