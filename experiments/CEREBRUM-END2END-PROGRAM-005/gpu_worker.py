...
def runtime_preflight(parent):
    ...
    audit["repair_to_ordinary_sequence_ratio"]=(audit["datasets"]["train-repair"]["sequence_tokens_per_pass"]/
...
def train(arm,parent):
    ...
    previously_started = (out/"start.json").exists()
    ...
    if previously_started and last is None:
        raise ValueError("previous training start has no resumable checkpoint; audit before any restart")
    if last:
        ...
        step = read(Path(last)/"trainer_state.json")["global_step"]
        if not 0 < step <= cfg()["max_steps"]:
            raise ValueError("checkpoint step outside registered budget")
...
def read_prefix(path,expected_ids):
    ...
    for i,b in enumerate(parts):
        try:r=parse(b.decode())
            ...
            break
        if not isinstance(r, dict) or "case_id" not in r:
            raise ValueError("prediction record is not an identified object")
...
def predict(arm,split,parent):
    verify()
    if split != "development":raise ValueError("Program-005 is development-only")
    if arm not in (*cfg()["arms"],"parent"):raise ValueError("unknown model")
    if (ROOT/"results/confirmation-access.json").exists():raise ValueError("development closed")
