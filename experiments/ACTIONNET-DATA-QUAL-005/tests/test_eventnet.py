...
class EventNetTests(unittest.TestCase):
    ...
    def test_fresh_families_and_validation_renderer(self):
        train = self.generated["datasets"]["train"]
        validation = self.generated["datasets"]["repair_validation"]
        self.assertEqual({row["metadata"]["semantic_family"] for row in train}, set(range(70, 82)))
        self.assertEqual({row["metadata"]["semantic_family"] for row in validation}, set(range(90, 94)))
        self.assertEqual({row["metadata"]["selected_renderer"] for row in validation}, {"AUDIT_PACKET"})
        self.assertEqual(self.generated["audits"]["actionnet_004_case_overlap"], 0)
        self.assertEqual(self.generated["audits"]["actionnet_004_prompt_overlap"], 0)
    ...
    def test_pivotal_weights_and_effective_decision_balance(self):
        ...
        self.assertEqual(set(pivotal_pair_weights), {3.0})
