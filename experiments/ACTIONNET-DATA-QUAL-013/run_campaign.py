#!/usr/bin/env python3
"""Materialize and qualify ACTIONNET-DATA-QUAL-013."""
...
from actionnet013 import DATASET_ID, PROTOCOL_ID, RESULT_ID, canonical, generate
...
def main() -> int:
    ...
    paths = {
        ...
        "selection": ROOT / "dataset" / "development_selection.jsonl",
    ...
    write_jsonl(paths["selection"], generated["datasets"]["development_selection"])
    ...
    write_json(paths["lineages"], {"schema_version": "actionnet-lineage-registry.v13", "records": generated["lineages"]})
    write_json(paths["reservation"], generated["protected"])
    status = "READY_FOR_CEREBRUM_DEV013_BOUNDARY_SELECTION" if all(controls.values()) else "HOLD_FOR_REPAIR"
    report = {
        "schema_version": "actionnet-data-qualification-report.v13",
        ...
        "claim_boundary": (
            "Fresh project-authored synthetic near-clock development selection and untouched confirmation data only; "
            "no full regression, independent transfer, real-institution, production, authority, or IGI claim."
        ),
    }
    ...
    print(json.dumps({
        ...
        "selection_records": len(generated["datasets"]["development_selection"]),
