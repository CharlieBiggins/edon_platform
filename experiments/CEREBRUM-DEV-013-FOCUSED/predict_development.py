#!/usr/bin/env python3
"""Freeze development predictions for all preregistered checkpoint candidates."""
...
from dev013_common import BASE, CONFIG, ROOT, candidate_specs, read_json, sha256_path, sha256_tree, write_json
...
def main() -> int:
    ...
    prepare()
    development = ROOT / "prepared" / "development-selection-32.jsonl"
    records = []
    for candidate in candidate_specs(config):
        ...
        manifest.update({
            "schema_version": "cerebrum-dev013-development-prediction-manifest.v1",
        ...
        records.append({
            ...
            "complete": manifest["count"] == 32,
        })
    freeze = {
        "schema_version": "cerebrum-dev013-development-prediction-freeze.v1",
        "protocol_id": config["protocol_id"],
