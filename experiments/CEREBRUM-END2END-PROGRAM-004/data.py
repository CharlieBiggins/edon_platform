...
RULES = (
    ...
    "MASKED_RESOURCE_PREDICATE", "SUBMISSION_ORDER_INVARIANCE",
)
EXCLUDED = set()
ACQUISITION_AUDITS = []
...
def ordinary(split, start, families, seed, deps):
    ...
    records, trajectories, _, _, _ = gen.make_split(
        ...
        tuple(f"curriculum004-{split}-domain-{i}" for i in range(8)), gen.renderer_lineage())
...
def boundary_pair(template, family, index, split, seed, deps):
    ...
    elif rule == "MASKED_RESOURCE_PREDICATE":
        mechanism = "DECISIVE_CONTEXT_PRESERVATION"
        base["request"]["well_formed"] = False
        path, left, right = ("state", "resources", "capacity"), base["resources"]["demand"] + 2, base["resources"]["demand"] + 1
    else:
        mechanism = "QUEUE_ORDER_AND_ACTOR_RENAME"
        path, left, right = ("presentation",), False, True
    ...
    for variant, value in enumerate((left, right)):
        ...
        trajectory.update(semantic_family=family, counterfactual_pair_id=pair,
            ...
            pair_class="INVARIANCE" if index == 7 else ("CONTEXTUAL" if index == 6 else "PIVOTAL"))
    ...
    if index == 6:
        da,db = [trace.build_trace(r["compiler_input"]["program_source"])["derivation"] for r in result]
        if da["blocked_checks"][-1] == db["blocked_checks"][-1]:
            raise ValueError("masked resource predicate did not change")
    return result
...
def generate(split, deps):
    ...
    if split == "train":
        ...
        return {"train-ordinary": control, "train-boundary": treatment}
...
def prepare():
    if (ROOT / "results/confirmation-access.json").exists():
        raise ValueError("preparation closed after confirmation access")
    from audit_parent import audit
    audit()
    ...
    for split in ("train", "development"):
        ...
        old.extend(values)
    deps = dependencies()  # Reset generator namespace after parent reconstruction.
    ...
    write_json(ROOT / "prepared/qualification.json", {"passed":True, "splits":report,
        ...
        "parent_train_development_excluded":len(old), "confirmation_accessed":False,
