...
EXPERIMENT = "CEREBRUM-END2END-PROGRAM-004"
...
def verify(workspace, parent_search):
    ...
    require(read(old / "registration.json")["sources"] == reg["inherited_sources"], "Inherited source manifest differs")
    ...
    for name, count in (("train-ordinary", 512), ("train-boundary", 512), ("development", 256)):
    ...
    for arm in ("ordinary", "boundary"):
        ...
        require(m["step"] == config["max_steps"] == 32, "Incomplete training: " + arm)
        adapters[arm] = tree_hash(directory / "adapter")
        require(adapters[arm] == m["adapter_sha256"], "Trained adapter mismatch: " + arm)
        print(arm.upper() + ": STEP 32 ADAPTER VERIFIED", flush=True)
    ...
    require(len(ids) == 256 and len(set(ids)) == 256, "Development IDs invalid")
    for arm in ("ordinary", "boundary", "parent"):
        ...
        print(f"{arm.upper()}: {len(records)}/256 saved responses", flush=True)
        if partial:
...
def main():
    ...
    verify(args.workspace, args.parent_search)
