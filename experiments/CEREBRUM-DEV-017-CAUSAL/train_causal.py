#!/usr/bin/env python3
"""Continue DEV-015 step 12 for one complete causal-chain curriculum pass."""
...
from dev017_common import read_jsonl, sha256_path, sha256_tree
...
def main() -> int:
    ...
    expected_count = config["registered_condition_counts"][config["primary_condition"]]
    if len(rows) != expected_count:
        raise SystemExit(f"prepared training count differs from preregistration: {len(rows)} != {expected_count}")
    registered_exposures = int(config["effective_batch_size"]) * int(config["max_steps"])
    if registered_exposures != len(rows) or registered_exposures != int(config["registered_effective_sample_exposures"]):
        raise SystemExit("registered optimizer schedule does not expose the complete curriculum exactly once")
    if float(config["epochs"]) != 1.0:
        raise SystemExit("complete-exposure protocol requires exactly one registered epoch")
    ...
    class CompletionDataset(Dataset):
        def __init__(self, source: list[dict[str, Any]]):
            ...
            completion_by_task: dict[str, list[int]] = {}
            ...
            for row in source:
                ...
                task = row["task_type"]
                completion_by_task.setdefault(task, []).append(completion_len)
                if completion_len + int(config["generation_token_margin"]) > int(config["task_token_limits"][task]):
                    limit_violations.append(row["case_id"])
            ...
            if limit_violations:
                raise ValueError(f"task generation limits too small for {len(limit_violations)} training records")
            self.audit = {
                "examples": len(source),
                "max_full_tokens": max(full_lengths),
                "max_completion_tokens": max(completion_lengths),
                "max_completion_tokens_by_task": {task: max(values) for task, values in sorted(completion_by_task.items())},
                "prompt_truncation_examples": truncations,
                "generation_limit_violations": len(limit_violations),
    ...
    manifest = {
        "schema_version": "cerebrum-dev017-causal-training-manifest.v1",
        ...
        "fresh_causal_repair_training": True,
        "delayed_evidence_safety_emphasis": True,
        "exact_state_reconstruction_emphasis": True,
        "complete_causal_chain_supervision": True,
        "mutation_locality_supervision": True,
        "sampler_policy": "seeded_random_permutation_without_replacement",
        "registered_complete_dataset_pass": True,
        "registered_effective_sample_exposures": registered_exposures,
        "registered_exposure_fraction": registered_exposures / len(rows),
        ...
        "claim_boundary": "One complete fresh synthetic causal-chain curriculum pass only; no confirmed full regression, transfer, production, authority, or IGI claim.",
    }
