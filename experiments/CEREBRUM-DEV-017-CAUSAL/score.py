#!/usr/bin/env python3
"""Score the single-use 192-case DEV-017 confirmation."""

from __future__ import annotations

import json

from dev017_common import CONFIG, ROOT, read_json, read_jsonl, sha256_path, write_json
...
def main() -> int:
    ...
    if not candidate_id:
        result = {
            "schema_version": "cerebrum-dev017-causal-result.v1",
        ...
        write_json(ROOT / "results" / "dev017-causal-result.json", result)
    ...
    result = {
        ...
        "metrics": metrics,
        "passed": passed,
        "status": "CAUSAL_REPAIR_FULL_REGRESSION_SIGNAL" if passed else "CAUSAL_REPAIR_FULL_REGRESSION_HOLD",
        ...
        "public_scored": False,
        "binding_authority": False,
        "claim_boundary": "One adaptively selected causal-repair candidate on one fresh project-authored synthetic full confirmation; no seed reproducibility, independent transfer, production, authority, or IGI claim.",
    }
    write_json(ROOT / "results" / "dev017-causal-result.json", result)
    print(json.dumps(result, indent=2, sort_keys=True))
    return 0
