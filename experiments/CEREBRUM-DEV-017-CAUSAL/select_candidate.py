...
from dev017_common import CONFIG, ROOT, candidate_specs, read_json, read_jsonl, sha256_path, sha256_tree, write_json
...
def choose_candidate(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    return max(
        eligible,
        key=lambda row: (
            -row["metrics"]["certificate"]["unsafe_authorizations"],
            row["metrics"]["transition"]["post_state_exact"],
            row["metrics"]["certificate"]["pair_behavior"].get("PIVOTAL", {}).get("rate", 0.0),
...
def main() -> int:
    ...
    result = {
        "schema_version": "cerebrum-dev017-development-selection.v1",
