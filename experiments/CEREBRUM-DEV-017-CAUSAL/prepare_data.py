#!/usr/bin/env python3
"""Prepare fresh causal-repair training, selection, and confirmation data."""
...
from dev017_common import ACTIONNET, ROOT, canonical, read_json, read_jsonl, sha256_path, summarize, write_json, write_jsonl
...
SYSTEM_PROMPT = (
    ...
    "routing, conflict, authority, and appeal finality. Preserve every state field not explicitly targeted by an "
    "executed mutation. Deferred evidence cannot satisfy a current decision, and an executed withdrawal removes its "
    "approval. Never treat an appeal as resolved unless its resolve event executes at or before the clock. Derive the "
    "decision and certificate only after reconstructing the final state, and keep all requested views consistent. "
    "Return only the exact requested compact JSON with binding_authority=false. "
...
def prepare() -> dict[str, Any]:
    ...
    if qualification.get("status") != "READY_FOR_CEREBRUM_DEV017_CAUSAL_REPAIR":
        raise ValueError("ACTIONNET-DATA-QUAL-017 is not qualified")
    sources = {
    ...
    manifest = {
        "schema_version": "cerebrum-dev017-causal-data-manifest.v1",
        ...
        "training_records": len(prepared["train"]),
        "effective_batch_size": 16,
        "optimizer_steps": 24,
        "registered_effective_sample_exposures": 384,
        "registered_full_dataset_exposure": len(prepared["train"]) == 16 * 24 == 384,
        "curriculum_order_preserved": [row["case_id"] for row in prepared["train"]] == [
            row["case_id"] for row in read_jsonl(sources["train"])
        ],
        "dev014_dev015_or_dev016_validation_cases_used_for_training": False,
