...
CONFIG = ROOT / "configs" / "dev015-mixed.json"
EXPERIMENTS = ROOT.parent
ACTIONNET = EXPERIMENTS / "ACTIONNET-DATA-QUAL-015"
BASE = EXPERIMENTS / "CEREBRUM-DEV-009"
PARENT = EXPERIMENTS / "CEREBRUM-DEV-013-FOCUSED"
...
def parent_artifact_root(config: dict[str, Any]) -> Path:
    return PARENT / "artifacts" / "dev013-parent-dev012-step12-seed-26090532"


def parent_adapter(config: dict[str, Any]) -> Path:
    return parent_artifact_root(config) / f"checkpoint-{config['parent_selected_step']}"


def artifact_root(config: dict[str, Any]) -> Path:
    return ROOT / "artifacts" / "dev015-parent-dev013-step6-seed-26090542"


def candidate_specs(config: dict[str, Any]) -> list[dict[str, Any]]:
    root = artifact_root(config)
    return [{**candidate, "adapter": root / f"checkpoint-{candidate['step']}"} for candidate in config["development_candidates"]]
