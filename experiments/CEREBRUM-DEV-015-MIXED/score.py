#!/usr/bin/env python3
"""Score the single-use 192-case DEV-015 confirmation."""

from __future__ import annotations

import json

from dev015_common import CONFIG, ROOT, read_json, read_jsonl, sha256_path, write_json
...
def main() -> int:
    ...
    if not candidate_id:
        result = {
            "schema_version": "cerebrum-dev015-mixed-result.v1",
        ...
        write_json(ROOT / "results" / "dev015-mixed-result.json", result)
    ...
    result = {
        ...
        "metrics": metrics,
        "passed": passed,
        "status": "MIXED_RETENTION_FULL_REGRESSION_SIGNAL" if passed else "MIXED_RETENTION_FULL_REGRESSION_HOLD",
        ...
        "public_scored": False,
        "binding_authority": False,
        "claim_boundary": "One adaptively selected mixed-replay candidate on one fresh project-authored synthetic full confirmation; no seed reproducibility, independent transfer, production, authority, or IGI claim.",
    }
    write_json(ROOT / "results" / "dev015-mixed-result.json", result)
    print(json.dumps(result, indent=2, sort_keys=True))
    return 0
