...
class EventNetTests(unittest.TestCase):
    ...
    def test_registered_counts_tasks_and_renderers(self):
        ...
        self.assertEqual(len(train), 16128)
        self.assertEqual(len(validation), 1344)
        self.assertEqual(len(self.generated["canonical_trajectories"]), 1536)
    ...
    def test_fresh_families_and_validation_renderer(self):
        train = self.generated["datasets"]["train"]
        validation = self.generated["datasets"]["repair_validation"]
        self.assertEqual({row["metadata"]["semantic_family"] for row in train}, set(range(180, 204)))
        self.assertEqual({row["metadata"]["semantic_family"] for row in validation}, set(range(220, 228)))
        self.assertEqual({row["metadata"]["selected_renderer"] for row in validation}, {"CROSS_FORMAT_REGISTER"})
        self.assertEqual(self.generated["audits"]["actionnet_006_case_overlap"], 0)
        self.assertEqual(self.generated["audits"]["actionnet_006_prompt_overlap"], 0)

    def test_decision_clock_deferral_is_structural(self):
        for split, expected in (("train", 1152), ("repair_validation", 384)):
