#!/usr/bin/env python3
"""Predict the untouched full confirmation only for a passing candidate."""
...
from dev016_common import BASE, CONFIG, ROOT, artifact_root, candidate_specs, read_json, sha256_path, sha256_tree, write_json
...
def main() -> int:
    ...
    if not candidate_id:
        print("No checkpoint passed the development full gate; confirmation remains sealed.")
        return 0
    candidates = {item["candidate_id"]: item for item in candidate_specs(config)}
    candidate = candidates[candidate_id]
    adapter = candidate["adapter"]
    if sha256_tree(adapter) != selection.get("selected_adapter_tree_sha256"):
        raise SystemExit("selected adapter changed after development selection")
    confirmation = ROOT / "prepared" / "untouched-confirmation-192.jsonl"
    ...
    manifest.update({
        "schema_version": "cerebrum-dev016-confirmation-prediction-manifest.v1",
    ...
    freeze = {
        "schema_version": "cerebrum-dev016-confirmation-prediction-freeze.v1",
        "protocol_id": config["protocol_id"],
        "selected_candidate_id": candidate_id,
        "selected_step": candidate["step"],
        "selection_sha256": sha256_path(selection_path),
