#!/usr/bin/env python3
"""Launch or resume the registered 12-step narrow continuation."""
...
def main() -> int:
    ...
    required = (adapter, parent_manifest, parent_predictions, parent_selection, parent_result)
    ...
    if final_manifest.is_file() and (output / "final-adapter").is_dir() and all(
        (output / f"checkpoint-{step}").is_dir() for step in (6, 12)
    ):
        print(json.dumps(read_json(final_manifest), indent=2, sort_keys=True))
        print("DEV-016 narrow continuation already complete; existing checkpoints retained.")
        return 0
    command = [
        sys.executable,
        str(ROOT / "train_narrow.py"),
        "--config",
        str(CONFIG),
        "--seed",
    ...
    if checkpoint is not None:
        ...
        print(f"Resuming DEV-016 from {checkpoint}", flush=True)
