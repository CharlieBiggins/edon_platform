#!/usr/bin/env python3
"""Governed multi-domain ActionNet authoring generator.

ACTIONNET-DATA-QUAL-008 is additive to the frozen v7 execution-repair corpus.
It expands the executable mechanism core, compiles a broad registry of domain
packs into deterministic synthetic review candidates, and attaches the source,
expert, privacy, lineage, and release gates required before any candidate can
become future Cerebrum training data.  The generated rows are deliberately
named authoring candidates: no row is training eligible until human and
governance gates are satisfied outside this synthetic package.
"""
...
PROTOCOL_ID = "ACTIONNET-DATA-QUAL-008"
RESULT_ID = "ACTIONNET-DATA-QUAL-008-result-v1.0.0"
DATASET_ID = "ACTIONNET-GOVERNED-MULTIDOMAIN-AUTHORING-v8.0.0"
TRAIN_SEED = 26081908
VALIDATION_SEED = 26081909
TRAIN_RENDERERS = (
    "DECISION_CLOCK_GRID",
    "MULTI_SYSTEM_JOURNAL",
    "STATE_DELTA_PACKET",
    "QUEUE_CONTROL_SHEET",
)
...
PIVOTAL_MECHANISMS = LEGACY_PIVOTAL_MECHANISMS + NEW_PIVOTAL_MECHANISMS
...
def load_domain_registry() -> dict[str, Any]:
    return json.loads((ROOT / "registry" / "domains.json").read_text(encoding="utf-8"))


DOMAIN_REGISTRY = load_domain_registry()
DOMAIN_PACKS = tuple(DOMAIN_REGISTRY["domains"])
DOMAIN_BY_ID = {item["domain_id"]: item for item in DOMAIN_PACKS}
...
def institution_config(family_id: int, domain: str, institution_index: int) -> dict[str, Any]:
    domain_pack = DOMAIN_BY_ID[domain]
    scopes = ("authorize", "procure", "enroll", "credential", "review", "allocate")
    workflow = domain_pack["workflows"][family_id % len(domain_pack["workflows"])]
    return {
        "family_id": family_id,
        "domain": domain,
        "domain_pack_version": "1.0.0",
        "sector_group": domain_pack["sector_group"],
        "risk_tier": domain_pack["risk_tier"],
        "workflow_name": workflow,
        "jurisdiction": domain_pack["jurisdictions"][family_id % len(domain_pack["jurisdictions"])],
        "expert_specialties": list(domain_pack["expert_specialties"]),
        "evidence_types": list(domain_pack["evidence_types"]),
        "registered_mechanisms": list(domain_pack["mechanisms"]),
...
def initial_state(config: dict[str, Any], pair_index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "consent": {"required": True, "active": True},
        "decision_body": {"quorum_required": 2, "votes_present": 2},
        "custody": {"chain_required": True, "chain_intact": True},
        "delegation_condition": {"required": True, "satisfied": True},
        "obligation": {
            "reporting_required": True,
            "deadline": query_time - 1,
            "reported_at": query_time - 2,
        },
        "budget": {"available": config["capacity"] + demand + 2, "encumbered": 0, "required": demand},
        "quality": {"hold_active": False, "recall_required": False, "recall_complete": True},
        "domain_context": {
            "domain_id": config["domain"],
            "domain_pack_version": config["domain_pack_version"],
            "sector_group": config["sector_group"],
...
def pivotal_events(mechanism: str, state: dict[str, Any], pair_id: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    elif mechanism == "CONSENT_WITHDRAWAL":
        base.append(event(10, "SET_CONSENT_ACTIVE", "consent.active", True, actor=requester))
        changed.append(event(10, "SET_CONSENT_ACTIVE", "consent.active", False, actor=requester))
    elif mechanism == "QUORUM_FAILURE":
        required = state["decision_body"]["quorum_required"]
        base.append(event(10, "SET_VOTES_PRESENT", "decision_body.votes_present", required, actor=approver))
        changed.append(event(10, "SET_VOTES_PRESENT", "decision_body.votes_present", required - 1, actor=approver))
    elif mechanism == "CHAIN_OF_CUSTODY_BREAK":
        base.append(event(10, "SET_CUSTODY_INTACT", "custody.chain_intact", True, actor=reviewer))
        changed.append(event(10, "SET_CUSTODY_INTACT", "custody.chain_intact", False, actor=reviewer))
    elif mechanism == "CONDITIONAL_DELEGATION":
        base.append(event(10, "SET_DELEGATION_CONDITION", "delegation_condition.satisfied", True, actor=delegator))
        changed.append(event(10, "SET_DELEGATION_CONDITION", "delegation_condition.satisfied", False, actor=delegator))
    elif mechanism == "REPORTING_DEADLINE":
        deadline = state["obligation"]["deadline"]
        base.append(event(10, "SET_REPORTED_AT", "obligation.reported_at", deadline, actor=reviewer))
        changed.append(event(10, "SET_REPORTED_AT", "obligation.reported_at", None, actor=reviewer))
    elif mechanism == "BUDGET_ENCUMBRANCE":
        base.append(event(10, "SET_BUDGET_ENCUMBERED", "budget.encumbered", 0, actor=approver))
        changed.append(event(10, "SET_BUDGET_ENCUMBERED", "budget.encumbered", state["budget"]["available"], actor=approver))
    elif mechanism == "QUALITY_HOLD":
        base.append(event(10, "SET_QUALITY_HOLD", "quality.hold_active", False, actor=reviewer))
        changed.append(event(10, "SET_QUALITY_HOLD", "quality.hold_active", True, actor=reviewer))
    elif mechanism == "RECALL_INCOMPLETION":
        state["quality"]["recall_required"] = True
        base.append(event(10, "SET_RECALL_COMPLETE", "quality.recall_complete", True, actor=reviewer))
        changed.append(event(10, "SET_RECALL_COMPLETE", "quality.recall_complete", False, actor=reviewer))
    else:
...
def transition_a(state: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    ...
    if operation in {
        ...
        "SET_VOTES_PRESENT",
        "SET_CUSTODY_INTACT",
        "SET_DELEGATION_CONDITION",
        "SET_REPORTED_AT",
        "SET_BUDGET_ENCUMBERED",
        "SET_QUALITY_HOLD",
        "SET_RECALL_COMPLETE",
    }:
...
def transition_b(state: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    ...
    direct_paths = {
        ...
        "SET_CONSENT_ACTIVE": ("consent", "active"),
        "SET_VOTES_PRESENT": ("decision_body", "votes_present"),
        "SET_CUSTODY_INTACT": ("custody", "chain_intact"),
        "SET_DELEGATION_CONDITION": ("delegation_condition", "satisfied"),
        "SET_REPORTED_AT": ("obligation", "reported_at"),
        "SET_BUDGET_ENCUMBERED": ("budget", "encumbered"),
        "SET_QUALITY_HOLD": ("quality", "hold_active"),
        "SET_RECALL_COMPLETE": ("quality", "recall_complete"),
    }
...
def evaluate_a(state: dict[str, Any]) -> dict[str, Any]:
    ...
    decision_body = state["decision_body"]
    custody = state["custody"]
    delegation_condition = state["delegation_condition"]
    ...
    elif delegation_condition["required"] and not delegation_condition["satisfied"]:
        semantic, decision, failed = "FALSE", "DENY", ["DELEGATION_CONDITION"]
    elif consent["required"] and not consent["active"]:
        semantic, decision, failed = "FALSE", "DENY", ["CONSENT"]
    ...
    elif custody["chain_required"] and not custody["chain_intact"]:
        semantic, decision, failed = "FALSE", "DENY", ["EVIDENCE_CUSTODY"]
    ...
    elif decision_body["votes_present"] < decision_body["quorum_required"]:
        semantic, decision, failed = "UNKNOWN", "ABSTAIN", ["QUORUM"]
    elif obligation["reporting_required"] and obligation["deadline"] <= query_time and (
        obligation["reported_at"] is None or obligation["reported_at"] > obligation["deadline"]
    ):
        semantic, decision, failed = "UNKNOWN", "ABSTAIN", ["REPORTING_OBLIGATION"]
    elif budget["required"] > budget["available"] - budget["encumbered"]:
        semantic, decision, failed = "UNKNOWN", "ABSTAIN", ["BUDGET_CAPACITY"]
    elif quality["hold_active"]:
        semantic, decision, failed = "FALSE", "DENY", ["QUALITY_HOLD"]
    elif quality["recall_required"] and not quality["recall_complete"]:
        semantic, decision, failed = "FALSE", "DENY", ["RECALL_INCOMPLETE"]
...
def evaluate_b(state: dict[str, Any]) -> dict[str, Any]:
    ...
    factors = {
        ...
        "delegation_condition": state["delegation_condition"]["required"] and not state["delegation_condition"]["satisfied"],
        "consent": state["consent"]["required"] and not state["consent"]["active"],
        ...
        "custody": state["custody"]["chain_required"] and not state["custody"]["chain_intact"],
        ...
        "quorum": state["decision_body"]["votes_present"] < state["decision_body"]["quorum_required"],
        "reporting": state["obligation"]["reporting_required"] and state["obligation"]["deadline"] <= now and (
            state["obligation"]["reported_at"] is None or state["obligation"]["reported_at"] > state["obligation"]["deadline"]
        ),
        "budget": state["budget"]["required"] > state["budget"]["available"] - state["budget"]["encumbered"],
        "quality_hold": state["quality"]["hold_active"],
        "recall": state["quality"]["recall_required"] and not state["quality"]["recall_complete"],
    ...
    ordered = (
        ...
        ("quorum", "UNKNOWN", "ABSTAIN", ["QUORUM"]),
        ("reporting", "UNKNOWN", "ABSTAIN", ["REPORTING_OBLIGATION"]),
        ("budget", "UNKNOWN", "ABSTAIN", ["BUDGET_CAPACITY"]),
        ("quality_hold", "FALSE", "DENY", ["QUALITY_HOLD"]),
        ("recall", "FALSE", "DENY", ["RECALL_INCOMPLETE"]),
    ...
    records = [
        {
            ...
            "version": "1.0.0",
            "content_sha256": digest({"style": style, "schema": "actionnet-event-render-v8"}),
            "parents": [],
            "authority": "EDON Research Lab",
            "transformation": "render-governed-multidomain-eventnet-v8",
        }
...
def generate_split(
    ...
    lineages.append({
        ...
        "parents": ["ACTIONNET-DATA-QUAL-006-result-v1.0.0"],
        "authority": "EDON Research Lab",
    ...
    domain_mechanism_counts: Counter[str] = Counter()
    ...
    for institution_index, family_id in enumerate(families):
        ...
        risk_counts[domain_pack["risk_tier"]] += 1
        source_lineage = opaque("source", PROTOCOL_ID, split, seed, domain, institution_index)
        domain_pack_lineage = opaque("domain-pack", PROTOCOL_ID, domain, "1.0.0")
        ...
        lineages.extend([
            {
                "lineage_id": domain_pack_lineage,
                "lineage_type": "domain_pack",
                "version": "1.0.0",
                "content_sha256": digest(domain_pack),
                "parents": [],
                "authority": "EDON Research Lab",
                "transformation": "project-authored-domain-pack-v1",
            ...
            },
            {
                ...
                "parents": [],
                "authority": "EDON Research Lab",
                "transformation": "project-authored-synthetic-source-v8",
            ...
            },
            {
                ...
                "parents": [source_lineage, domain_pack_lineage],
                "authority": "EDON Research Lab",
                "transformation": "compile-governed-domain-institution-v8",
            },
            {
        ...
        for local_pair in range(pairs_per_institution):
            ...
            if pair_class == "PIVOTAL":
                registered = tuple(config["registered_mechanisms"])
                mechanism = registered[local_pair % len(registered)]
                pivotal_counter += 1
            ...
            domain_mechanism_counts[f"{domain}:{mechanism}"] += 1
            ...
            for variant, variant_state, events, intervention_family in variants:
                ...
                trajectory = {
                    ...
                    "split": split,
                    "domain": domain,
                    "domain_pack_lineage": domain_pack_lineage,
                    "workflow_name": config["workflow_name"],
                    "jurisdiction": config["jurisdiction"],
                    "risk_tier": config["risk_tier"],
                    "expert_specialties": config["expert_specialties"],
    ...
    audit = {
        ...
        "domain_mechanism_distribution": dict(domain_mechanism_counts),
...
def metadata(trajectory: dict[str, Any], style: str, task_type: str, sample_weight: float) -> dict[str, Any]:
    return {
        "domain_id": trajectory["domain"],
        "domain_pack_lineage": trajectory["domain_pack_lineage"],
        "workflow_name": trajectory["workflow_name"],
        "jurisdiction": trajectory["jurisdiction"],
        "risk_tier": trajectory["risk_tier"],
        "expert_specialties": trajectory["expert_specialties"],
...
def historical_overlap(case_ids: set[str], prompt_hashes: set[str]) -> dict[str, int]:
    ...
    spec = importlib.util.spec_from_file_location("actionnet_data_qual_007_history", predecessor)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load ActionNet-007 for historical-overlap audit")
    ...
    return {
        "actionnet_007_case_overlap": len(case_ids & previous_case_ids),
        "actionnet_007_prompt_overlap": len(prompt_hashes & previous_prompt_hashes),
    }


def generate() -> dict[str, Any]:
    renderer_ids, lineages = renderer_lineages()
    domain_ids = tuple(item["domain_id"] for item in DOMAIN_PACKS)
    train_trajectories, train_pairs, train_lineages, train_audit = generate_split(
        "authoring", TRAIN_SEED, tuple(range(300, 300 + len(domain_ids))), 6, domain_ids
    )
    validation_trajectories, validation_pairs, validation_lineages, validation_audit = generate_split(
        "heldout_authoring_validation", VALIDATION_SEED,
        tuple(range(340, 340 + len(domain_ids))), 6, tuple(reversed(domain_ids))
    )
    lineages.extend(train_lineages + validation_lineages)
    lineages = [
        record for _, record in sorted(
            {record["lineage_id"]: record for record in lineages}.items()
        )
    ]
    ...
    protected = {
        ...
        "multidomain_authoring_semantic_families": list(range(300, 300 + len(domain_ids))),
        "multidomain_validation_semantic_families": list(range(340, 340 + len(domain_ids))),
        "future_public_semantic_families": list(range(380, 388)),
        "protected_semantic_families": list(range(390, 398)),
    ...
    expected_train_pairs = len(domain_ids) * 6
    expected_validation_pairs = len(domain_ids) * 6
    ...
    controls = {
        "registered_counts": (
            len(train_records) == expected_train_records
            and len(validation_records) == expected_validation_records
            and len(all_trajectories) == 2 * (expected_train_pairs + expected_validation_pairs)
            and len(all_pairs) == expected_train_pairs + expected_validation_pairs
        ),
        ...
        "pair_diff_supervision_present": len(pair_train_records) == expected_train_pairs * len(TRAIN_RENDERERS),
        ...
        "future_public_and_protected_unmaterialized": not (set(range(380, 388)) | set(range(390, 398))) & {item["semantic_family"] for item in all_trajectories},
        "fresh_generator_lineages": train_audit["generator_lineage"] != validation_audit["generator_lineage"],
        "all_registered_domains_materialized": set(domain_ids) == set(train_audit["domain_distribution"]) == set(validation_audit["domain_distribution"]),
        "all_domain_mechanism_assignments_materialized": all(
            train_audit["domain_mechanism_distribution"].get(f"{item['domain_id']}:{mechanism}", 0) > 0
            for item in DOMAIN_PACKS for mechanism in item["mechanisms"]
        ),
        "all_candidates_blocked_from_training": all(
            row["metadata"]["training_eligible"] is False
            and row["metadata"]["expert_review_status"] == "PENDING"
            for row in all_records
        ),
        "all_trajectories_have_governance_envelope": all(
            trajectory["governance"]["training_eligible"] is False
            and trajectory["governance"]["source_grounded"] is False
            for trajectory in all_trajectories
        ),
        "lineage_ids_unique": len(lineages) == len({record["lineage_id"] for record in lineages}),
    }
    return {
        "datasets": {"authoring_candidates": train_records, "heldout_authoring_validation": validation_records},
