#!/usr/bin/env python3
"""Materialize and qualify ACTIONNET-DATA-QUAL-018."""
...
from actionnet018 import DATASET_ID, PROTOCOL_ID, RESULT_ID, canonical, generate
...
def main() -> int:
    predecessor = ROOT.parent / "ACTIONNET-DATA-QUAL-017" / "results" / "qualification_report.json"
    if not predecessor.is_file():
        raise SystemExit("missing ACTIONNET-DATA-QUAL-017 qualification report")
    report = json.loads(predecessor.read_text(encoding="utf-8"))
    if report.get("status") != "READY_FOR_CEREBRUM_DEV017_CAUSAL_REPAIR":
        raise SystemExit("ACTIONNET-DATA-QUAL-017 is not qualified")
    generated = generate()
    repeated = generate()
    ...
    write_json(paths["lineages"], {"schema_version": "actionnet-lineage-registry.v18", "records": generated["lineages"]})
    write_json(paths["reservation"], generated["protected"])
    status = "READY_FOR_CEREBRUM_DEV018_VERIFIED_HYBRID" if all(controls.values()) else "HOLD_FOR_REPAIR"
    qualification = {
        "schema_version": "actionnet-data-qualification-report.v18",
        ...
        "claim_boundary": "Fresh project-authored synthetic development and sealed confirmation for target-blind verified-hybrid integration only; no learned-capability, independent-transfer, production, authority, or IGI claim.",
    }
    report_path = ROOT / "results" / "qualification_report.json"
    write_json(report_path, qualification)
