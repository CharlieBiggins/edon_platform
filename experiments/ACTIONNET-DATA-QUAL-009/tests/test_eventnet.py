...
class EventNetTests(unittest.TestCase):
    ...
    def test_registered_counts_tasks_and_renderers(self):
        ...
        self.assertEqual(len(train), 10752)
        self.assertEqual(len(validation), 1344)
        self.assertEqual(len(self.generated["canonical_trajectories"]), 1920)
    ...
    def test_fresh_families_and_validation_renderer(self):
        train = self.generated["datasets"]["train"]
        validation = self.generated["datasets"]["repair_validation"]
        self.assertEqual({row["metadata"]["semantic_family"] for row in train}, set(range(300, 324)))
        self.assertEqual({row["metadata"]["semantic_family"] for row in validation}, set(range(340, 346)))
        self.assertEqual({row["metadata"]["selected_renderer"] for row in validation}, {"DEPENDENCY_GRAPH_PACKET"})
        self.assertEqual({row["metadata"]["generator_profile"] for row in train}, {"LEDGER", "MATRIX"})
        self.assertEqual({row["metadata"]["generator_profile"] for row in validation}, {"GRAPH"})
        self.assertEqual(self.generated["audits"]["actionnet_007_case_overlap"], 0)
        self.assertEqual(self.generated["audits"]["actionnet_007_prompt_overlap"], 0)

    def test_decision_clock_deferral_is_structural(self):
        for split, expected in (("train", 1536), ("repair_validation", 384)):
            audit = self.generated["audits"][split]
            self.assertEqual(audit["deferred_event_trajectories"], expected)
            self.assertEqual(audit["decision_clock_violations"], 0)
            self.assertEqual(set(audit["query_time_distribution"]), {8, 9, 10, 11, 12})
            self.assertGreaterEqual(min(audit["deferred_event_count_distribution"]), 1)
            self.assertGreaterEqual(min(audit["executed_event_count_distribution"]), 5)

    def test_every_trajectory_mutates_multiple_state_paths(self):
        for split in ("train", "repair_validation"):
            distribution = self.generated["audits"][split]["mutation_path_count_distribution"]
            self.assertGreaterEqual(min(distribution), 2)
    ...
    def test_pivotal_weights_and_effective_decision_balance(self):
        ...
        self.assertEqual(set(pivotal_pair_weights), {5.0})
        ...
        self.assertGreater(task_weight_sums["QUEUE_TRACE"], task_weight_sums["TRANSITION"])
