...
export function assertRuntimeProfile(profile: RuntimeProfile, dependencies: RuntimeDependencies) {
  ...
  if (profile !== 'STAGING_TEST' && (!(dependencies.receiptSigner instanceof KmsReceiptCustody) || dependencies.signingKeyMode !== 'KMS')) throw new Error(`${profile} requires KMS receipt custody`);
...
export function requireRuntimeEnvironment(env: Record<string, string | undefined>) {
  const required = ['RUNTIME_PROFILE', 'DATABASE_URL', 'MIGRATOR_ROLE', 'APPLICATION_ROLE', 'OIDC_ISSUER', 'OIDC_AUDIENCE', 'OIDC_JWKS_URL', 'AUDIT_LOGGING', 'TENANT_ISOLATION_ENFORCEMENT', 'API_HOST', 'API_PORT', 'CORS_ORIGIN'];
  ...
  if (env.RUNTIME_PROFILE === 'STAGING_TEST' && !env.RECEIPT_SIGNING_TEST_KEY) throw new Error('STAGING_TEST requires RECEIPT_SIGNING_TEST_KEY');
  ...
  return { profile: env.RUNTIME_PROFILE as RuntimeProfile, host: env.API_HOST!, port: Number(env.API_PORT), databaseUrl: env.DATABASE_URL!, oidcIssuer: env.OIDC_ISSUER!, oidcAudience: env.OIDC_AUDIENCE!, jwksUrl: env.OIDC_JWKS_URL!, signingKey: env.RECEIPT_SIGNING_TEST_KEY ?? '', kmsKeyId: env.KMS_KEY_ID, corsOrigin: env.CORS_ORIGIN! };
}
