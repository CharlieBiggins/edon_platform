...
if (process.env.WORKER_TEST_FAIL_AT && config.profile !== 'STAGING_TEST') throw new Error('Worker test fault injection is only permitted in STAGING_TEST');
...
const testFailurePoint = process.env.WORKER_TEST_FAIL_AT;
const failAt = (point: string) => { if (config.profile === 'STAGING_TEST' && testFailurePoint === point) throw new Error(`TEST_WORKER_FAULT_${point}`); };
...
const compileInstitution = async (message: OutboxMessage) => {
  ...
  if (ordered.length === 0) findings.push({ code: 'NO_SOURCES', institution_id: institutionId });
  for (const source of ordered) {
    ...
    if (source.classification_status === 'PENDING') findings.push({ code: 'SOURCE_UNCLASSIFIED', source_id: source.source_id, source_version: source.source_version });
  }
