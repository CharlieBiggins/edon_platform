...
export function createControlPlaneServer(options: ControlPlaneServerOptions = {}) {
  ...
  return createServer(async (request, response) => {
    ...
    if (authenticatedRoute && !principal) return failure(response, 'PERMISSION_DENIED', 'Authentication expired or revoked');
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/v1/reconstructions/')) return reply(response, 200, { data: { incident_id: url.pathname.split('/').at(-1), events: await runtimeDependencies.repositories.getEvents(tenantId!, url.pathname.split('/').at(-1)), state: await state(runtimeDependencies.repositories, tenantId!, 'memphis-fulfillment') }, meta: { correlation_id: 'read', contract_version: CONTRACT_VERSION } });
