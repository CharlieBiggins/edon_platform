...
export function createControlPlaneServer(options: ControlPlaneServerOptions = {}) {
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'POST' && url.pathname === '/v1/proposals') { const evidence = new DeterministicEvidenceAdmission().admit({ event: 'capacity_disruption' }); const proposal = new LogisticsProposalGenerator().create(currentState, evidence); await runtimeDependencies.repositories.putProposal({ ...proposal, tenant_id: bind!.tenant_id }); return reply(response, 201, { data: proposal, meta: { correlation_id: bind!.correlation_id, contract_version: CONTRACT_VERSION } }); }
