...
export function createControlPlaneServer(options: ControlPlaneServerOptions = {}) {
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/v1/incidents/')) return reply(response, 200, { data: { incident_id: url.pathname.split('/').at(-1), state: await state(runtimeDependencies.repositories, 'meridian-demo', 'memphis-fulfillment'), proposal_ids: [] }, meta: { correlation_id: 'read', contract_version: CONTRACT_VERSION } });
    if (request.method === 'GET' && url.pathname.startsWith('/v1/receipts/')) return reply(response, 200, { data: await runtimeDependencies.repositories.getReceipt('meridian-demo', url.pathname.split('/').at(-1) ?? ''), meta: { correlation_id: 'read', contract_version: CONTRACT_VERSION } });
    if (request.method === 'GET' && url.pathname.startsWith('/v1/proposals/')) return reply(response, 200, { data: await runtimeDependencies.repositories.getProposal('meridian-demo', url.pathname.split('/').at(-1) ?? ''), meta: { correlation_id: 'read', contract_version: CONTRACT_VERSION } });
    if (request.method === 'GET' && url.pathname.startsWith('/v1/decisions/')) return reply(response, 200, { data: await runtimeDependencies.repositories.getKernelDecision('meridian-demo', url.pathname.split('/').at(-1) ?? ''), meta: { correlation_id: 'read', contract_version: CONTRACT_VERSION } });
    if (request.method === 'GET' && url.pathname.startsWith('/v1/outcomes/')) return reply(response, 200, { data: await runtimeDependencies.repositories.getOutcome('meridian-demo', url.pathname.split('/').at(-1) ?? ''), meta: { correlation_id: 'read', contract_version: CONTRACT_VERSION } });
