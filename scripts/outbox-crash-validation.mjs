...
const workerEnv = { ...process.env, WORKER_PORT: '8789', WORKER_ID: `crash-harness-${process.pid}`, WORKER_LEASE_MS: '1000', WORKER_MAX_ATTEMPTS: '3', WORKER_TENANTS: tenant, WORKER_ACTOR_ID: 'outbox-worker-ci', WORKER_PAUSE_BEFORE_HANDLE_MS: '0', WORKER_PAUSE_BEFORE_COMPLETE_MS: '0' };
...
const waitReady = async () => { for (let attempt = 0; attempt < 40; attempt += 1) { try { const response = await fetch('http://127.0.0.1:8789/readyz'); if (response.ok) return; } catch {} await new Promise(resolve => setTimeout(resolve, 100)); } throw new Error('worker readiness timeout'); };
