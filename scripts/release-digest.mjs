...
const snapshot = {
  ...
  events: await rows("SELECT event_id,event_type,payload->>'actor_id' AS actor_id,payload->>'actor_type' AS actor_type,payload->>'occurred_at' AS occurred_at,recorded_at,(payload->>'state_version_before')::bigint AS state_version_before,(payload->>'state_version_after')::bigint AS state_version_after,payload->>'payload_hash' AS payload_hash,payload->>'previous_record_hash' AS previous_record_hash,payload FROM journal_events WHERE tenant_id=$1 AND payload->>'incident_id'=$2 ORDER BY recorded_at,event_id", [tenant, releaseId]),
