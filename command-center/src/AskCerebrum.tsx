...
import { Activity, ArrowLeft, ArrowRight, Bookmark, Boxes, CheckCircle2, ChevronRight, Clock3, Columns2, Compass, Expand, FileCheck2, GitBranch, Link2, ListChecks, LockKeyhole, Maximize2, Minimize2, Minus, MoreHorizontal, Network, Plus, RotateCcw, Search, Send, ShieldCheck, Sparkles, TrendingUp, X } from 'lucide-react';
...
function Inspector({ tab, setTab, props, highlightId }: { tab: InspectorTab; setTab: (tab: InspectorTab) => void; props: AskCerebrumProps; highlightId?: string }) {
  ...
  return <section className="ask-inspector" aria-label="Investigation objects"><div className="ask-inspector-tabs" role="tablist">{tabs.map(([id, label, Icon]) => <button key={id} role="tab" aria-selected={tab === id} onClick={() => setTab(id)}><Icon size={14} />{label}</button>)}</div><div className="ask-inspector-body" role="tabpanel">
    {tab === 'evidence' && <><div className="ask-inspector-title"><div><span className="eyebrow">ACCESSIBLE EVIDENCE</span><h3>Evidence inspector</h3></div><Badge tone="amber">2 unresolved</Badge></div>{evidence.map(item => <button className={`ask-evidence-row ${highlightId === item.id ? 'citation-highlight' : ''}`} key={item.id} onClick={() => { setSelectedEvidenceId(item.id); props.onShowEvidence(); }}><span className={`ask-evidence-dot ${item.status.toLowerCase()}`} /><span><strong>{item.title}</strong><small>{item.source}  -  {item.id}  -  {item.version}</small></span><Badge tone={item.status === 'Verified' ? 'teal' : item.status === 'Restricted' ? 'neutral' : 'amber'}>{item.status}</Badge></button>)}{selectedEvidence && <aside className="evidence-record-detail"><div className="evidence-detail-heading"><h3>{selectedEvidence.title}</h3><Badge tone={selectedEvidence.status === 'Verified' ? 'teal' : 'amber'}>{selectedEvidence.status}</Badge></div><dl><div><dt>Evidence ID</dt><dd>{selectedEvidence.id}</dd></div><div><dt>Source</dt><dd>{selectedEvidence.source}</dd></div><div><dt>Version</dt><dd>{selectedEvidence.version}</dd></div><div><dt>Observed</dt><dd>10:40:31 CDT</dd></div><div><dt>State binding</dt><dd>v{props.version}</dd></div><div><dt>Integrity</dt><dd>Verified</dd></div></dl><strong>Related commitments</strong><p>COM-1042-01 ? Protected<br/>COM-1042-02 ? Protected<br/>COM-1042-03 ? At risk<br/>COM-1042-04 ? At risk</p><div className="evidence-detail-actions"><button onClick={() => props.onNotify('This evidence has been attached to the simulated Cerebrum conversation.')}>Ask Cerebrum about this</button><button onClick={() => setTab('timeline')}>View timeline</button></div></aside>}</>}
...
export default function AskCerebrum(props: AskCerebrumProps) {
  ...
  useEffect(() => {
    function keydown(event: KeyboardEvent) { if (event.key === 'Escape' && props.open) { if (contextDrawerOpen) closeContextDrawer(); else mode === 'full' ? setMode('expanded') : props.onOpenChange(false); } }
  ...
  function openInspector(tab: InspectorTab, citation?: string) {
    if (typeof document !== 'undefined' && document.activeElement instanceof HTMLElement) drawerTriggerRef.current = document.activeElement;
    setInspectorTab(tab);
  ...
  return <div className={`ask-layer ask-${mode}`} data-mode={mode}><button className="ask-scrim" aria-label="Close Ask Cerebrum" onClick={() => props.onOpenChange(false)} /><section className="ask-shell density-comfortable" role="dialog" aria-modal="false" aria-label={workspaceLabel}>
    ...
    </section>{layout === 'WORKSPACE_FOCUS' && <aside className="workspace-context-rail" aria-label="Workspace navigation">{([['evidence', Search, 'Evidence'], ['timeline', Activity, 'Timeline'], ['commitments', ListChecks, 'Commitments'], ['plans', Columns2, 'Plans'], ['graph', Network, 'Control Graph'], ['decision', ShieldCheck, 'Decision'], ['outcome', TrendingUp, 'Outcomes'], ['receipts', FileCheck2, 'Receipts']] as const).map(([tab, Icon, label]) => <button key={tab} title={label} aria-label={label} aria-pressed={inspectorTab === tab} onClick={() => setInspectorTab(tab)}><Icon size={17}/><span>{label}</span>{(tab === 'evidence' || tab === 'commitments') && <i>{tab === 'evidence' ? 2 : props.incident.commitments}</i>}</button>)}</aside>}{mode !== 'docked' && <><button className="workspace-divider" type="button" role="separator" aria-orientation="vertical" aria-label="Resize Ask Cerebrum and institutional workspace" aria-valuemin={35} aria-valuemax={70} aria-valuenow={Math.round(chatWidth)} onPointerDown={event => event.currentTarget.setPointerCapture(event.pointerId)} onPointerMove={resizeFromPointer} onPointerUp={event => { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); }} onDoubleClick={() => selectPreset('balanced')} onKeyDown={keyboardResize}><span /></button><Inspector tab={inspectorTab} setTab={setInspectorTab} props={props}/></>}</div>
    {mode !== 'docked' && layout === 'CHAT_FOCUS' && <aside className="context-rail" aria-label="Institutional context rail">{([['evidence', Search, 'Evidence'], ['timeline', Activity, 'Timeline'], ['commitments', ListChecks, 'Commitments'], ['plans', Columns2, 'Plans'], ['graph', Network, 'Control Graph'], ['decision', ShieldCheck, 'Decision'], ['outcome', TrendingUp, 'Outcome'], ['receipts', FileCheck2, 'Receipts']] as const).map(([tab, Icon, label]) => <button key={tab} title={label} aria-label={`Open ${label}`} onClick={() => openInspector(tab)} disabled={tab === 'receipts' && !props.receipt}><Icon size={16}/><span>{label}</span>{(tab === 'evidence' || tab === 'commitments') && <i>{tab === 'evidence' ? 2 : props.incident.commitments}</i>}</button>)}</aside>}
    {mode !== 'docked' && layout === 'CHAT_FOCUS' && contextDrawerOpen && <div className="context-drawer-backdrop" role="presentation" onClick={closeContextDrawer}><aside ref={drawerRef} className="context-drawer" style={{ width: `clamp(400px, ${drawerWidth}px, 65vw)` }} role="dialog" aria-label={`${inspectorTab} context`} onClick={event => event.stopPropagation()}><button className="context-drawer-resize" type="button" role="separator" aria-orientation="vertical" aria-label="Resize context drawer" onPointerDown={event => event.currentTarget.setPointerCapture(event.pointerId)} onPointerMove={resizeDrawer} onPointerUp={event => { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); }}><span /></button><div className="context-drawer-actions"><strong>Institutional context</strong><button onClick={closeContextDrawer} aria-label="Close context drawer"><X size={17}/></button></div><Inspector tab={inspectorTab} setTab={openInspector} props={props} highlightId={highlightId}/></aside></div>}
    </div>
