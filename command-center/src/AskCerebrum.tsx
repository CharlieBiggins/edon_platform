...
function SummaryObject({ incident, onGuide }: { incident: Incident; onGuide: (tab: InspectorTab) => void }) {
  return <div className="ask-summary-card"><p>The Memphis disruption threatens four delivery commitments and creates approximately ${Math.round(incident.exposure / 1000)}K in modeled exposure.</p><div className="ask-summary-metrics"><span><strong>${Math.round(incident.exposure / 1000)}K</strong><small>modeled exposure</small></span><span><strong>{incident.commitments}</strong><small>commitments</small></span><span><strong>{incident.timeRemaining}</strong><small>remaining</small></span><span><strong>2</strong><small>assumptions</small></span></div><div className="ask-summary-actions"><button onClick={() => onGuide('commitments')}>View commitments <ArrowRight size={13} /></button><button onClick={() => onGuide('plans')}>Compare plans <ArrowRight size={13} /></button></div></div>;
}
