...
export default function AskCerebrum(props: AskCerebrumProps) {
  ...
  return <div className={`ask-layer ask-${mode}`} data-mode={mode}><button className="ask-scrim" aria-label="Close Ask Cerebrum" onClick={() => props.onOpenChange(false)} /><section className={`ask-shell density-${density}`} role="dialog" aria-modal="false" aria-label={workspaceLabel}>
    ...
    {mode !== 'docked' && collapsedPane === 'workspace' && <aside className="context-rail" aria-label="Institutional context rail">{([['evidence', Search, 'Evidence'], ['timeline', Activity, 'Timeline'], ['commitments', ListChecks, 'Commitments'], ['plans', Columns2, 'Plans'], ['graph', Network, 'Control Graph'], ['decision', ShieldCheck, 'Decision'], ['outcome', TrendingUp, 'Outcome'], ['receipts', FileCheck2, 'Receipts']] as const).map(([tab, Icon, label]) => <button key={tab} aria-label={`Open ${label}`} onClick={() => openInspector(tab)}><Icon size={16}/><span>{label}</span></button>)}<button className="context-rail-restore" onClick={() => restoreSplit()}><Columns2 size={16}/><span>Restore split</span></button></aside>}
    {mode !== 'docked' && collapsedPane === 'workspace' && contextDrawerOpen && <div className="context-drawer-backdrop" role="presentation" onClick={() => setContextDrawerOpen(false)}><aside className="context-drawer" role="dialog" aria-label={`${inspectorTab} context`} onClick={event => event.stopPropagation()}><div className="context-drawer-actions"><strong>Institutional context</strong><button onClick={() => setContextDrawerOpen(false)} aria-label="Close context drawer"><X size={17}/></button></div><Inspector tab={inspectorTab} setTab={openInspector} props={props}/><div className="context-drawer-footer"><button onClick={() => restoreSplit(inspectorTab)}>Restore split view</button><button className="primary" onClick={() => { setCollapsedPane(null); setContextDrawerOpen(false); setMode('full'); }}>Open dedicated view</button></div></aside></div>}
    </div>
