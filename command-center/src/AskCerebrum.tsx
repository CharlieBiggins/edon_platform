...
import { Activity, ArrowLeft, ArrowRight, Bookmark, Boxes, CheckCircle2, ChevronRight, Clock3, Columns2, Compass, Expand, FileCheck2, Focus, GitBranch, Link2, ListChecks, LockKeyhole, Maximize2, Minimize2, Minus, Network, Plus, RotateCcw, Search, Send, ShieldCheck, Sparkles, TrendingUp, Users, X } from 'lucide-react';
...
type IntentMode = 'ask' | 'analyze' | 'guide';
type AnalysisKind = 'explanation' | 'summary' | 'plans' | 'evidence' | 'timeline' | 'commitments' | 'proposal' | 'graph' | 'outcome' | 'guidance';
type Message = { id: number; role: 'user' | 'cerebrum'; text: string; kind?: AnalysisKind; intent?: IntentMode };
...
function responseMeta(message: Message) {
  if (message.intent === 'guide' || message.kind === 'guidance') return { label: 'PLATFORM GUIDANCE', tone: 'guidance' as const, Icon: Compass };
...
export default function AskCerebrum(props: AskCerebrumProps) {
  ...
  const [intent, setIntent] = useState<IntentMode>('ask');
  ...
  function submit(question = input) {
    ...
    const kind = selectedIntent === 'guide' ? 'guidance' : classify(clean);
    setMessages(prev => [...prev, { id: messageId.current++, role: 'user', text: clean, intent: selectedIntent }, { id: messageId.current++, role: 'cerebrum', text: answer(kind, props.incident, props.plan), kind, intent: selectedIntent }]);
    setInput('');
    setIntent('ask');
