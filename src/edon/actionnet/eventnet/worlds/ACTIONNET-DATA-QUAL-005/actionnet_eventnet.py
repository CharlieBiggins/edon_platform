#!/usr/bin/env python3
"""Fresh-lineage EventNet repair-data generator for CEREBRUM-DEV-004.

The v5 corpus retains the independently checked ActionNet-004 scheduler,
...
PROTOCOL_ID = "ACTIONNET-DATA-QUAL-005"
RESULT_ID = "ACTIONNET-DATA-QUAL-005-result-v1.0.0"
DATASET_ID = "ACTIONNET-EVENTNET-REPAIR-DATASET-v5.0.0"
TRAIN_SEED = 26081205
VALIDATION_SEED = 26081206
TRAIN_RENDERERS = ("FORMAL", "EVENT_STREAM", "CASE_DOCKET")
VALIDATION_RENDERER = "AUDIT_PACKET"
    ...
    records = [
        {
            ...
            "version": "1.0.0",
            "content_sha256": digest({"style": style, "schema": "actionnet-event-render-v5"}),
            "parents": [],
            "authority": "EDON Research Lab",
            "transformation": "render-eventnet-v5",
        }
...
def generate_split(
    ...
    lineages.append({
        ...
        "parents": ["ACTIONNET-DATA-QUAL-004-result-v1.0.0"],
        "authority": "EDON Research Lab",
        ...
        lineages.extend([
            {
                ...
                "parents": [],
                "authority": "EDON Research Lab",
                "transformation": "project-authored-synthetic-source-v5",
            },
            {
                ...
                "authority": "EDON Research Lab",
                "transformation": "compile-institution-v5",
            },
            {
        ...
        for style in styles:
            ...
            for task_type, query, target, task_multiplier in tasks:
                records.append({
                    ...
                    "target": target,
                    "metadata": metadata(trajectory, style, task_type, round(weight * task_multiplier, 6)),
                })
    for pair in pairs:
...
def historical_overlap(case_ids: set[str], prompt_hashes: set[str]) -> dict[str, int]:
    predecessor = ROOT.parent / "ACTIONNET-DATA-QUAL-004" / "actionnet_eventnet.py"
    spec = importlib.util.spec_from_file_location("actionnet_data_qual_004_history", predecessor)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load ActionNet-004 for historical-overlap audit")
    ...
    return {
        "actionnet_004_case_overlap": len(case_ids & previous_case_ids),
        "actionnet_004_prompt_overlap": len(prompt_hashes & previous_prompt_hashes),
    }
...
def generate() -> dict[str, Any]:
    ...
    train_trajectories, train_pairs, train_lineages, train_audit = generate_split(
        "train", TRAIN_SEED, tuple(range(70, 82)), 20, ("licensing", "grants", "records")
    )
    validation_trajectories, validation_pairs, validation_lineages, validation_audit = generate_split(
        "repair_validation", VALIDATION_SEED, tuple(range(90, 94)), 15, ("benefits-administration", "infrastructure-administration")
    )
    ...
    protected = {
        ...
        "future_public_semantic_families": [94, 95, 96, 97],
        "protected_semantic_families": [98, 99, 100, 101],
    ...
    controls = {
        ...
        "actionnet_004_not_reused": audits["actionnet_004_case_overlap"] == 0 and audits["actionnet_004_prompt_overlap"] == 0,
        ...
        "future_public_and_protected_unmaterialized": not ({94, 95, 96, 97, 98, 99, 100, 101} & {item["semantic_family"] for item in all_trajectories}),
