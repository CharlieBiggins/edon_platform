#!/usr/bin/env python3
"""Fresh-lineage execution-transfer repair generator for CEREBRUM-DEV-007.
...
PROTOCOL_ID = "ACTIONNET-DATA-QUAL-007"
RESULT_ID = "ACTIONNET-DATA-QUAL-007-result-v1.0.0"
DATASET_ID = "ACTIONNET-EXECUTION-TRANSFER-REPAIR-DATASET-v7.0.0"
TRAIN_SEED = 26081607
VALIDATION_SEED = 26081608
TRAIN_RENDERERS = (
    "DECISION_CLOCK_GRID",
    ...
    records = [
        {
            ...
            "version": "1.0.0",
            "content_sha256": digest({"style": style, "schema": "actionnet-event-render-v7"}),
            "parents": [],
...
def historical_overlap(case_ids: set[str], prompt_hashes: set[str]) -> dict[str, int]:
    predecessor = ROOT.parent / "ACTIONNET-DATA-QUAL-006" / "actionnet_eventnet.py"
    spec = importlib.util.spec_from_file_location("actionnet_data_qual_006_history", predecessor)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load ActionNet-006 for historical-overlap audit")
    ...
    return {
        "actionnet_006_case_overlap": len(case_ids & previous_case_ids),
        "actionnet_006_prompt_overlap": len(prompt_hashes & previous_prompt_hashes),
    }
...
def generate() -> dict[str, Any]:
    ...
    train_trajectories, train_pairs, train_lineages, train_audit = generate_split(
        "train", TRAIN_SEED, tuple(range(180, 204)), 24,
        ("permit-control", "procurement-routing", "research-operations", "service-allocation", "credential-review")
    )
    validation_trajectories, validation_pairs, validation_lineages, validation_audit = generate_split(
        "repair_validation", VALIDATION_SEED, tuple(range(220, 228)), 24,
        ("records-disposition", "resource-jurisdiction", "cross-system-review")
    )
    ...
    protected = {
        ...
        "future_public_semantic_families": list(range(240, 248)),
        "protected_semantic_families": list(range(250, 258)),
    ...
    controls = {
        "registered_counts": len(train_records) == expected_train_records and len(validation_records) == expected_validation_records and len(all_trajectories) == 1536 and len(all_pairs) == 768,
        ...
        "actionnet_006_not_reused": audits["actionnet_006_case_overlap"] == 0 and audits["actionnet_006_prompt_overlap"] == 0,
        ...
        "future_public_and_protected_unmaterialized": not (set(range(240, 248)) | set(range(250, 258))) & {item["semantic_family"] for item in all_trajectories},
