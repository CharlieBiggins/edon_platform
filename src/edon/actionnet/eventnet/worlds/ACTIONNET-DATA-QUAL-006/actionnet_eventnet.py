#!/usr/bin/env python3
"""Fresh-lineage unresolved-appeal repair generator for CEREBRUM-DEV-006.

The v6 corpus retains the independently checked ActionNet-005 scheduler,
transition, and oracle semantics while changing every generated case, lineage,
semantic family, domain, and validation renderer.  Targets are deliberately
compact: cryptographic digests are derived downstream from model-predicted
states instead of being emitted as language-model tokens.
"""
...
PROTOCOL_ID = "ACTIONNET-DATA-QUAL-006"
RESULT_ID = "ACTIONNET-DATA-QUAL-006-result-v1.0.0"
DATASET_ID = "ACTIONNET-APPEAL-REPAIR-DATASET-v6.0.0"
TRAIN_SEED = 26081406
VALIDATION_SEED = 26081407
TRAIN_RENDERERS = ("CONTROL_BRIEF", "TIMELINE_LEDGER", "PROCESS_PACKET")
VALIDATION_RENDERER = "REVIEW_MEMORANDUM"
...
def pivotal_events(mechanism: str, state: dict[str, Any], pair_id: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if mechanism == "REVOCATION_PROPAGATION":
        ...
        open_time = (3, 5, 7)[timing_profile]
        resolved_time = (6, 8, 9)[timing_profile]
        unresolved_time = (11, 12, 14)[timing_profile]
        base.extend([
            event(3, "OPEN_APPEAL", "workflow.appeal_open", True, time=open_time, actor=requester),
            event(4, "RESOLVE_APPEAL", "workflow.appeal_open", False, time=resolved_time, actor=reviewer),
        ])
        changed.extend([
            event(3, "OPEN_APPEAL", "workflow.appeal_open", True, time=open_time, actor=requester),
            event(4, "RESOLVE_APPEAL", "workflow.appeal_open", False, time=unresolved_time, actor=reviewer),
        ])
    else:
...
def render(style: str, domain: str, state: dict[str, Any], events: list[dict[str, Any]]) -> tuple[str, str]:
    ...
    if style == "CONTROL_BRIEF":
        queue = "\n".join(f"CONTROL_EVENT::{event_text(event)}" for event in events)
        return (
            "text/plain;profile=control-brief-v1",
            f"CONTROL_BRIEF domain={domain}\nSTATE={canonical(visible)}\nQUEUE_BEGIN\n{queue}\nQUEUE_END\n"
            f"DISPOSITION_TIME={state['request']['query_time']}",
        )
    if style == "TIMELINE_LEDGER":
        queue = " || ".join(event_text(event) for event in events)
        return (
            "text/plain;profile=timeline-ledger-v1",
            f"TIMELINE_LEDGER<{domain}> INITIAL::{canonical(visible)} SUBMITTED::{queue} "
            f"DECIDE_AT::{state['request']['query_time']}",
        )
    if style == "PROCESS_PACKET":
        return (
            "application/json;profile=process-packet-v1",
            canonical({
                "institution_domain": domain,
                "disposition_time": state["request"]["query_time"],
                "initial_process_state": visible,
                "submitted_process_events": events,
            }),
        )
    if style == "REVIEW_MEMORANDUM":
        queue = " ".join(f"REVIEW_ENTRY[{event_text(event)}]" for event in events)
        return (
            "text/plain;profile=review-memorandum-v1",
            f"Review memorandum for {domain}. Evaluate the typed initial record {canonical(visible)} at "
            f"disposition time {state['request']['query_time']}. The submitted entries are not guaranteed "
            f"to be chronological. {queue}",
        )
    if style == "FORMAL":
    ...
    records = [
        {
            ...
            "version": "1.0.0",
            "content_sha256": digest({"style": style, "schema": "actionnet-event-render-v6"}),
            "parents": [],
            "authority": "EDON Research Lab",
            "transformation": "render-eventnet-v6",
        }
...
def generate_split(
    ...
    lineages.append({
        ...
        "parents": ["ACTIONNET-DATA-QUAL-005-result-v1.0.0"],
        "authority": "EDON Research Lab",
        ...
        lineages.extend([
            {
                ...
                "parents": [],
                "authority": "EDON Research Lab",
                "transformation": "project-authored-synthetic-source-v6",
            },
            {
                ...
                "authority": "EDON Research Lab",
                "transformation": "compile-institution-v6",
            },
            {
        ...
        for style in styles:
            ...
            for task_type, query, target, task_multiplier in tasks:
                appeal_multiplier = (
                    3.0
                    if weighted
                    and task_type == "CERTIFICATE"
                    and trajectory["pair_mechanism"] == "UNRESOLVED_APPEAL"
                    else 1.0
                )
                records.append({
                    ...
                    "metadata": metadata(
                        ...
                        style,
                        task_type,
                        round(weight * task_multiplier * appeal_multiplier, 6),
                    ),
                })
    for pair in pairs:
        base_trajectory = pair["base"]
        comparison = pair["comparison"]
        pair_weight = (3.0 if pair["pair_class"] == "PIVOTAL" else 1.5) if weighted else 1.0
        if weighted and pair["intervention_family"] == "UNRESOLVED_APPEAL":
            pair_weight = 4.5
        for style in styles:
...
def historical_overlap(case_ids: set[str], prompt_hashes: set[str]) -> dict[str, int]:
    predecessor = ROOT.parent / "ACTIONNET-DATA-QUAL-005" / "actionnet_eventnet.py"
    spec = importlib.util.spec_from_file_location("actionnet_data_qual_005_history", predecessor)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load ActionNet-005 for historical-overlap audit")
    ...
    return {
        "actionnet_005_case_overlap": len(case_ids & previous_case_ids),
        "actionnet_005_prompt_overlap": len(prompt_hashes & previous_prompt_hashes),
    }
...
def generate() -> dict[str, Any]:
    ...
    train_trajectories, train_pairs, train_lineages, train_audit = generate_split(
        "train", TRAIN_SEED, tuple(range(110, 126)), 24,
        ("permit-review", "procurement-oversight", "research-administration", "service-eligibility")
    )
    validation_trajectories, validation_pairs, validation_lineages, validation_audit = generate_split(
        "repair_validation", VALIDATION_SEED, tuple(range(140, 148)), 24,
        ("records-review", "resource-governance", "credential-appeals")
    )
    ...
    appeal_train_certificates = [
        row for row in train_records
        if row["metadata"]["task_type"] == "CERTIFICATE"
        and row["metadata"]["pair_mechanism"] == "UNRESOLVED_APPEAL"
    ]
    appeal_validation_certificates = [
        row for row in validation_records
        if row["metadata"]["task_type"] == "CERTIFICATE"
        and row["metadata"]["pair_mechanism"] == "UNRESOLVED_APPEAL"
    ]
    appeal_validation_decisions = Counter(row["target"]["decision"] for row in appeal_validation_certificates)
    appeal_resolution_times = {
        split: sorted({
            event["time"]
            for trajectory in trajectories
            if trajectory["pair_mechanism"] == "UNRESOLVED_APPEAL"
            for event in trajectory["submitted_events"]
            if event["operation"] == "RESOLVE_APPEAL"
        })
        for split, trajectories in (
            ("train", train_trajectories),
            ("repair_validation", validation_trajectories),
        )
    }
    event_visibility = 0
    ...
    protected = {
        ...
        "future_public_semantic_families": list(range(150, 158)),
        "protected_semantic_families": list(range(160, 168)),
    ...
    audits = {
        ...
        "appeal_train_certificate_count": len(appeal_train_certificates),
        "appeal_validation_certificate_count": len(appeal_validation_certificates),
        "appeal_validation_decisions": dict(sorted(appeal_validation_decisions.items())),
        "appeal_resolution_times": appeal_resolution_times,
        **historical,
    }
    expected_train_pairs = 16 * 24
    expected_validation_pairs = 8 * 24
    ...
    controls = {
        "registered_counts": len(train_records) == expected_train_records and len(validation_records) == expected_validation_records and len(all_trajectories) == 1152 and len(all_pairs) == 576,
        ...
        "actionnet_005_not_reused": audits["actionnet_005_case_overlap"] == 0 and audits["actionnet_005_prompt_overlap"] == 0,
        "appeal_certificate_training_focus": len(appeal_train_certificates) >= 400,
        "appeal_certificate_validation_power": len(appeal_validation_certificates) >= 60,
        "appeal_allow_contested_hard_negative_balance": appeal_validation_decisions.get("ALLOW") == appeal_validation_decisions.get("CONTESTED") and set(appeal_validation_decisions) == {"ALLOW", "CONTESTED"},
        "appeal_timing_diversity": all(len(times) >= 6 for times in appeal_resolution_times.values()),
        ...
        "future_public_and_protected_unmaterialized": not (set(range(150, 158)) | set(range(160, 168))) & {item["semantic_family"] for item in all_trajectories},
